...
public class WizardController implements ActionListener
{
    ...
    public WizardController(Wizard w)
    {
     ...
     public WizardController(Wizard w, Boolean freely)
    {
        wizard = w;
        conn=Connect.ConnectDb();
        freely = true;
    ...
    private void nextButtonPressed() throws SQLException
    {
        ...
        if(descriptor.getClass()==TestPanel1Descriptor.class) {
           ...
           if (((CreditPanel0) descriptor.getPanelComponent()).noneisSelected()) {
               ...
               System.out.println("1. O looo");
               Object nextPanelDescriptor = descriptor.getNextPanelDescriptor();
               loanref=((CreditPanel0) descriptor.getPanelComponent()).getRef();
               System.out.println("nextPanelDescriptor.getClass()"+nextPanelDescriptor.getClass());
              
               if(((String) nextPanelDescriptor).equalsIgnoreCase("AUTOMATIC_DESCRIPTOR")) {
                   System.out.println("1. O.0 looo");
                    System.out.println("getclassautom"+nextPanelDescriptor.getClass());
                    
                       Date oriDate = ((CreditPanel0) descriptor.getPanelComponent()).getOriDate();
                       System.out.println("ori date vaut"+ oriDate);
                      ((CreditPanelAutom1) wizard.getModel().getPanelDescriptor(nextPanelDescriptor).getPanelComponent()).setDateOri(begindate);  
                      // ((CreditPanelAutom1) wizard.getModel().getCurrentPanelDescriptor().getPanelComponent()).setDateOri(begindate);  
                     ((CreditPanelAutom1) wizard.getModel().getPanelDescriptor(nextPanelDescriptor).getPanelComponent()).setLoanref(loanref); 

                 // ((CreditPanelAutom1) wizard.getModel().getCurrentPanelDescriptor().getPanelComponent()).setLoanref(loanref); 
               } else if(nextPanelDescriptor.getClass()==CreditPanelManuel1.class) 
                  {   
                      System.out.println("1. O.0.1 looo");
                      System.out.println("getclassnorm"+nextPanelDescriptor.getClass());
                      ((CreditPanelManuel1) wizard.getModel().getCurrentPanelDescriptor().getPanelComponent()).setLoanref(loanref); }
 ...
 //             ((CreditPanelManuel2) wizard.getModel().getCurrentPanelDescriptor().getPanelComponent()).fillTable(new Date(), 10, "periodique", 28);
              ...
              if (((CreditPanelAutom1) descriptor.getPanelComponent()).calculMensualite()) {
                  if(((CreditPanelAutom1) descriptor.getPanelComponent()).getFirstPayementDate()==null) {
                      JOptionPane.showMessageDialog(descriptor.getPanelComponent(), "Veuillez entrer la date du premier paiement");
                  } else if (((CreditPanelAutom1) descriptor.getPanelComponent()).getNbPeriod()==-1) {
                      JOptionPane.showMessageDialog(descriptor.getPanelComponent(), "Veuillez entrer le nombre de périodes");

                  }  else if (((CreditPanelAutom1) descriptor.getPanelComponent()).getrate()==0) {
                      JOptionPane.showMessageDialog(descriptor.getPanelComponent(), "Le taux d'intérêt ne peut nul ou vide"); 
                  } else {
